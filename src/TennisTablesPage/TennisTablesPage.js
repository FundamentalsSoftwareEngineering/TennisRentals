...
const TennisTablesPage = (props) => {

    const [showModal, setShowModal] = useState(false);
    const [showErrorMessage, setShowErrorMessage] = useState(false);
    const [showError, setShowError] = useState(true);
    const [showSuccessMessage, setShowSuccessMessage] = useState(false);
    const [selectedTennisTable, setSelectedTennisTable] = useState([]);
    const [day, setDay] = useState('');
    const [month, setMonth] = useState('');
    const [year, setYear] = useState('');
    const [startTime, setStartTime] = useState('');
    const [endTime, setEndTime] = useState('');
    const { user, tennisTables, reserve, reservations } = props;
    const handleClose = () => {
        setShowModal(false);
    };
    const handleShow = (item) => {
        setSelectedTennisTable(item);
        setShowModal(true);
    };
    const handleMonth = (e) => {
    ...
    const checkAvailability= () => {
        reservations.map(item => {
            if (item.startTime === startTime && item.selectedCourt === selectedTennisTable.name &&
        ...
        <div className="my-3 my-md-5">
            ...
            <div className="container">
                    ...
                    <div className="container">
                        <div className="row">
                            <div className="col-md-12" >
                                ...
                                <h2>{selectedTennisTable.name}</h2>
                                <p>{selectedTennisTable.location}, {selectedTennisTable.address}</p>
                                <p>{selectedTennisTable.price}</p>
                                <hr/>
                                {
                                    showErrorMessage &&
                                        <div>
                                    <h3> Not available for selected date-time! </h3>
                                            <img src='https://toppng.com/uploads/preview/red-x-in-circle-x-ico-11563249170jvl0jhe7df.png'
                                                 style={{width:30, float:'left'}} alt='x'/>
                                        </div>
                                }
                                {
                                    showError &&
                                ...
                                <img src={selectedTennisTable.imageUrl} style={{width:400}}/>
                                    ...
                                    <button className="btn btn-primary" style={{float: 'right', width: 100}}
                                            onClick={() => {
                                                if (showErrorMessage === false && showError === false) {
                                                    props.reserve(user.id, selectedTennisTable.name, day, month, year, startTime, endTime);
                                                    handleClose();
                ...
                <div className="page-header">
                    ...
                    {
                        showSuccessMessage &&
                        <p> You have successfully rented table { selectedTennisTable.name }</p>
                    }
                </div>
                <div className="row row-cards">

                    {
                        tennisTables && tennisTables.map((item, i) => {
                                    ...
                                    <div className="card p-3">
                                        ...
                                        <div className="px-2">
                                            ...
                                            <div className="text-muted">
                                                ...
                                                <i className="col-sm-5">Price: </i> {item.price}
                                            </div>
...
function mapState(state) {
    const { authentication, tennisTables, reservations } = state;
    const { user } = authentication;
    return { user, tennisTables, reservations };
}
const mapDispatch = (dispatch) => ({
    reserve: (user, selectedTennisTable, day, month, year, startTime, endTime) => {dispatch(tennisCourtsActions.reserveAction(user, selectedTennisTable, day, month, year, startTime, endTime))}
});

const connectedTennisTablesPage = connect(mapState, mapDispatch)(TennisTablesPage);
export { connectedTennisTablesPage as TennisTablesPage };
